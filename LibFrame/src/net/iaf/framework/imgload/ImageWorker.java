...
public abstract class ImageWorker {
     ...
     * @param loadImgViaMobile 移动网络连接下，是否加载图片，true表示加载
     */
     ...
     */
    public void loadImage(Object data, ImageView imageView,
                          LoadingIndicatorView indicatorView, boolean loadImgViaMobile) {
        if (data == null || TextUtils.isEmpty(data.toString().trim())) {
            if (mLoadingBitmap != null && imageView != null)
                imageView.setImageBitmap(mLoadingBitmap);
            return;
        ...
        if (bitmap != null) {
            ...
            final BitmapWorkerTask task = new BitmapWorkerTask(imageView,
                    indicatorView, loadImgViaMobile);
            final AsyncDrawable asyncDrawable = new AsyncDrawable(mResources,
                    mLoadingBitmap, task);
            imageView.setImageDrawable(asyncDrawable);
    ...
    protected class BitmapWorkerTask extends AsyncTask<Object, Integer, Bitmap> {
        ...
        public BitmapWorkerTask(ImageView imageView,
                                LoadingIndicatorView indicatorView, boolean shouldLoadInMobile) {
            imageViewReference = new WeakReference<ImageView>(imageView);
            loadingIndicatorViewRef = new WeakReference<LoadingIndicatorView>(
                    indicatorView);
        ...
        protected Bitmap doInBackground(Object... params) {
            Loger.d(TAG + "doInBackground - starting work");
            ...
            if (bitmap == null && !isCancelled()
                ...
                if (!shouldLoadInMobile) {
                    if (!PhoneStateUtil.isUsingMobileConnect(BaseApplication
                            .getApplication())) {
                        bitmap = processBitmap(params[0], this);
                    } else {
                        Loger.d(TAG + "mobile状态下不加载图片");
                    }
                } else {
                    bitmap = processBitmap(params[0], this);
                }
            ...
            Loger.d(TAG + "doInBackground - finished work");
        ...
        private LoadingIndicatorView getAttachedIndicatorView() {
            if (loadingIndicatorViewRef != null) {
                final LoadingIndicatorView indicatorView = loadingIndicatorViewRef
                        .get();
                if (indicatorView != null) {
                    final AsyncIndicator indicator = indicatorView
                            .getAsyncIndicator();
                    if (indicator != null) {
                        final BitmapWorkerTask bitmapWorkerTask = indicator
                                .getBitmapWorkerTask();
                        if (this == bitmapWorkerTask) {
